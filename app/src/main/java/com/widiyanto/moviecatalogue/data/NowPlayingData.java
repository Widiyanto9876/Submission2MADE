...
public class NowPlayingData extends AsyncTaskLoader<ArrayList<MovieItem>> {
    private ArrayList<MovieItem> mData;
    private boolean mHasResult = false;
    private MovieItem nowplayingItems;

    public NowPlayingData(Context context) {
        super(context);
        onContentChanged();
    }
    ...
    public ArrayList<MovieItem> loadInBackground() {
        ...
        final ArrayList<MovieItem> nowPlayingItems = new ArrayList<>();
            ...
            public void onSuccess(int statusCode, Header[] headers, byte[] responseBody) {
                try {
                    ...
                    for (int i = 0; i < list.length(); i++){
                        ...
                        nowplayingItems = new MovieItem(catalugue);
                        nowPlayingItems.add(nowplayingItems);
